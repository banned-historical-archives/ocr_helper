...
export default async function (
  ...
  if ((opt as any).page_limits.length === 1) {
    return [(await parse_article((opt as any).page_limits[0]))!];
  }
  async function split_by_article(doc: pdfjsLib.Doc) {
    const res = [];
    let s = '';
    for (const range of (opt as any).page_limits) {
      for (let i = range[0]; i <= range[1]; ++i) {
  ...
  pages_by_article.push((opt as any).page_limits[(opt as any).page_limits.length - 1][1] + 1);
