...
export default async function (
  ...
  if ((parser_opt as any).header_min_height) {
    opt.header_min_height = (parser_opt as any).header_min_height;
  }
  if ((parser_opt as any).content_min_x) {
    opt.content_min_x = (parser_opt as any).content_min_x;
  }
  ...
  const parse_all = async function (
    ...
    content_objs = fix_before_parsing(content_objs, range[0], (parser_opt as any).name!);
    ...
    for (let i = 0, part_idx = 0; i < parts_raw.length; ++i, ++part_idx) {
      ...
      if ((part.type as any) === ContentType.title) {
        ...
        articles.push({
          title,
          description:
            (parser_opt as any).name === 'xuanji3' && title === '关于若干历史问题的决议'
  ...
  const parser_results = await parse_all((parser_opt as any).page_limits[0]);
